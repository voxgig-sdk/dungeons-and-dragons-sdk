...
require_relative 'DungeonsAndDragons_types'
...
class DungeonsAndDragonsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DungeonsAndDragonsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DungeonsAndDragonsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DungeonsAndDragonsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DungeonsAndDragonsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DungeonsAndDragonsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DungeonsAndDragonsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DungeonsAndDragonsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DungeonsAndDragonsError.new(
        "#{op}_allow",
        "DungeonsAndDragonsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue DungeonsAndDragonsError => err
    ...
    if fetched.is_a?(Hash)
      status = DungeonsAndDragonsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DungeonsAndDragonsError.new(
        "graphql_error", "DungeonsAndDragonsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = DungeonsAndDragonsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
