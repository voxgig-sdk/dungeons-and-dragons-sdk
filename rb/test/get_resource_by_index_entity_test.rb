...
class GetResourceByIndexEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DUNGEONSANDDRAGONS_TEST_GET_RESOURCE_BY_INDEX_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_resource_by_index_ref01_data_dt0_loaded, err = get_resource_by_index_ref01_ent.load(get_resource_by_index_ref01_match_dt0, nil)
    assert_nil err
    assert !get_resource_by_index_ref01_data_dt0_loaded.nil?
...
def get_resource_by_index_basic_setup(extra)
  ...
  entid_env_raw = ENV["DUNGEONSANDDRAGONS_TEST_GET_RESOURCE_BY_INDEX_ENTID"]
