...
class GetResourceListEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = DungeonsAndDragonsSDK.test(seed, nil)
    seen = base.GetResourceList(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.GetResourceList(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DUNGEONS_AND_DRAGONS_TEST_GET_RESOURCE_LIST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def get_resource_list_basic_setup(extra)
  ...
  entid_env_raw = ENV["DUNGEONS_AND_DRAGONS_TEST_GET_RESOURCE_LIST_ENTID"]
