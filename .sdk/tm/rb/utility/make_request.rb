...
module DungeonsAndDragonsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DungeonsAndDragonsResponse.new({})
    ...
    if fetch_err
      ...
      response = DungeonsAndDragonsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DungeonsAndDragonsResponse.new(fetched)
    else
