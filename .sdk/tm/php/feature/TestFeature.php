...
// DungeonsAndDragons SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class DungeonsAndDragonsTestFeature extends DungeonsAndDragonsBaseFeature
{
    ...
    public function init(DungeonsAndDragonsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (DungeonsAndDragonsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = DungeonsAndDragonsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (DungeonsAndDragonsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(DungeonsAndDragonsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = DungeonsAndDragonsParam::call($ctx, $k);
