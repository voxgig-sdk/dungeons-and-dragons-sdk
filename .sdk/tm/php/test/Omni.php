...
//    only - a typed DungeonsAndDragonsContext there would make every ctx
...
final class DungeonsAndDragonsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new DungeonsAndDragonsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new DungeonsAndDragonsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new DungeonsAndDragonsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new DungeonsAndDragonsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new DungeonsAndDragonsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?DungeonsAndDragonsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DungeonsAndDragonsError(is_string($code) ? $code : '', $msg);
    }
