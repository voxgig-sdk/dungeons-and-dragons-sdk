...
class DungeonsAndDragonsMakeRequest
{
    public static function call(DungeonsAndDragonsContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new DungeonsAndDragonsResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new DungeonsAndDragonsResponse($fetched);
        } else {
