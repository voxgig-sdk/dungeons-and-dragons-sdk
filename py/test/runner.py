# DungeonsAndDragons SDK test runner
...
class DungeonsAndDragonsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DungeonsAndDragonsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DungeonsAndDragonsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DungeonsAndDragonsTestRunner.getenv("DUNGEONS_AND_DRAGONS_TEST_LIVE")
        override = DungeonsAndDragonsTestRunner.getenv("DUNGEONS_AND_DRAGONS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DungeonsAndDragonsTestRunner.getenv(key)
        ...
        explain = DungeonsAndDragonsTestRunner.getenv("DUNGEONS_AND_DRAGONS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if DungeonsAndDragonsTestRunner._test_control is not None:
            return DungeonsAndDragonsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DungeonsAndDragonsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DungeonsAndDragonsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DungeonsAndDragonsTestRunner.load_test_control()
...
def load_env_local():
    DungeonsAndDragonsTestRunner.load_env_local()


def env_override(m):
    return DungeonsAndDragonsTestRunner.env_override(m)


def entity_data(v):
    return DungeonsAndDragonsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DungeonsAndDragonsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DungeonsAndDragonsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DungeonsAndDragonsTestRunner.load_test_control()


def live_client_options():
    return DungeonsAndDragonsTestRunner.live_client_options()


def live_delay_ms():
    return DungeonsAndDragonsTestRunner.live_delay_ms()
