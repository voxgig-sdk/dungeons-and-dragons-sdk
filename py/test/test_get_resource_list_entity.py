...
class TestGetResourceListEntity:

    def test_should_create_instance(self):
        testsdk = DungeonsAndDragonsSDK.test(None, None)
        ent = testsdk.GetResourceList(None)
    ...
    def test_should_stream(self):
        ...
        base = DungeonsAndDragonsSDK.test(seed, None)
        seen = list(base.GetResourceList(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = DungeonsAndDragonsSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.GetResourceList(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _get_resource_list_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "get_resource_list." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DUNGEONS_AND_DRAGONS_TEST_GET_RESOURCE_LIST_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        get_resource_list_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.get_resource_list")))
        get_resource_list_ref01_data = None
        if len(get_resource_list_ref01_data_raw) > 0:
            get_resource_list_ref01_data = helpers.to_map(get_resource_list_ref01_data_raw[0][1])

        # LIST
        get_resource_list_ref01_ent = client.GetResourceList(None)
...
def _get_resource_list_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/get_resource_list/GetResourceListTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DUNGEONS_AND_DRAGONS_TEST_GET_RESOURCE_LIST_ENTID"))
