# GetResourceByIndex direct test
...
class TestGetResourceByIndexDirect:

    def test_should_direct_load_get_resource_by_index(self):
        setup = _get_resource_by_index_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-get_resource_by_index", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_resource_by_index_direct_setup(mockres):
