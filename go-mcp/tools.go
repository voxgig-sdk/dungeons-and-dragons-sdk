...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_api_root | get_resource_by_index | get_resource_list | graph_ql"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.DungeonsAndDragonsSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "dungeons-and-dragons_list",
		Description: "List records from DungeonsAndDragons. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "dungeons-and-dragons_load",
		Description: "Load a single record from DungeonsAndDragons. " +
...
func runOp(client *sdk.DungeonsAndDragonsSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.DungeonsAndDragonsSDK, name string) (sdk.DungeonsAndDragonsEntity, error) {
	switch strings.ToLower(name) {
	case "get_api_root":
		return client.GetApiRoot(nil), nil
	case "get_resource_by_index":
		return client.GetResourceByIndex(nil), nil
	case "get_resource_list":
		return client.GetResourceList(nil), nil
	case "graph_ql":
