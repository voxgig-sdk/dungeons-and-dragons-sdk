# dungeons-and-dragons-mcp

[MCP](https://modelcontextprotocol.io) server exposing the DungeonsAndDragons SDK as
two agent tools — `dungeons-and-dragons_list` and `dungeons-and-dragons_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dungeons-and-dragons-mcp)
make build

# 2. Provide credentials via the environment
export DUNGEONS_AND_DRAGONS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dungeons-and-dragons \
  -- /absolute/path/to/dungeons-and-dragons-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dungeons-and-dragons-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dungeons-and-dragons_list: first page of records
{ "entity": "get_resource_list" }
{ "entity": "get_resource_list", "query": { } }

// dungeons-and-dragons_load: one record by id
{ "entity": "get_api_root", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dungeons-and-dragons-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dungeons-and-dragons \
     -- "$PWD"/dist/*/dungeons-and-dragons-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dungeons-and-dragons_list` and `dungeons-and-dragons_load` tools now appear
   in new sessions. Ask the agent to *"list get_resource_list using dungeons-and-dragons"*
   and it calls `dungeons-and-dragons_list` with `{"entity":"get_resource_list"}`.
...
```sh
export DUNGEONS_AND_DRAGONS_APIKEY=sk_live_xxx            # API key
export DUNGEONS_AND_DRAGONS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `dungeons-and-dragons_list` tool
...
### Call the `dungeons-and-dragons_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `dungeons-and-dragons_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dungeons-and-dragons_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DUNGEONS_AND_DRAGONS_APIKEY` | API key sent with every request. |
| `DUNGEONS_AND_DRAGONS_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

get_api_root | get_resource_by_index | get_resource_list | graph_ql

### Smoke test via HTTP (raw JSON-RPC)

```sh
./dungeons-and-dragons-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dungeons-and-dragons_load","arguments":{"entity":"get_api_root","query":{"id":1}}}}'
```
