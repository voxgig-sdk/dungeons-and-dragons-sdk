...
//     (\`new DungeonsAndDragonsSDK(...)\` / \`DungeonsAndDragonsSDK.test(...)\` become
//     \`DungeonsAndDragonsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'DungeonsAndDragonsSDK'
...
const TEST_SEED = {"entity":{"get_api_root":{"test01":{"id":"test01"}},"get_resource_by_index":{"test01":{"id":"test01"}},"get_resource_list":{"test01":{"id":"test01"}},"graph_ql":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DungeonsAndDragonsSDK(...)\` and any
// \`DungeonsAndDragonsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DungeonsAndDragonsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DungeonsAndDragonsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
