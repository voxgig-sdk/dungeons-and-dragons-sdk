...
describe('GetApiRootDirect', async () => {
  ...
  test('direct-load-get_api_root', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_api_root', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DUNGEONS_AND_DRAGONS_TEST_GET_API_ROOT_ENTID']
