...
describe('GetResourceListDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new DungeonsAndDragonsSDK({
  ...
  test('direct-list-get_resource_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_resource_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["get_resource_list01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DUNGEONS_AND_DRAGONS_TEST_GET_RESOURCE_LIST_ENTID']
