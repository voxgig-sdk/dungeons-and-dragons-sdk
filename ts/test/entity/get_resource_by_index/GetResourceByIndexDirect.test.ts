...
describe('GetResourceByIndexDirect', async () => {
  ...
  test('direct-load-get_resource_by_index', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_resource_by_index', setup.live)) return
    if (skipIfMissingIds(t, setup, ["index01","resource01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DUNGEONS_AND_DRAGONS_TEST_GET_RESOURCE_BY_INDEX_ENTID']
