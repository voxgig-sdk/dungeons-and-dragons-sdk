...
func main() {
    ...
    getapiroot, err := client.GetApiRoot(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil {
        panic(err)
    }
...
fmt.Println(getapiroot) // the loaded mock data
```
...
slice):
    ...
    getapiroot, err := client.GetApiRoot(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil { /* handle */ }
    // getapiroot is the loaded record
...
```go
get_resource_by_index, err := client.GetResourceByIndex(nil).Load(map[string]any{"id": "get_resource_by_index_id"}, nil)
if err != nil {
    panic(err)
}
...
getapiroot.Load(map[string]any{"id": "example_id"}, nil)
