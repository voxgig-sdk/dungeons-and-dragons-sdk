...
import (
	...
	omni "github.com/voxgig-sdk/dungeons-and-dragons-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DungeonsAndDragonsError); is {
				out := map[string]any{
					"name":    "DungeonsAndDragonsError",
...
func omniCtx(arg any, client *sdk.DungeonsAndDragonsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DungeonsAndDragonsSDK); is {
			client = live
