...
func TestGetResourceListEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetResourceList(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetResourceList(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		getResourceListRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_resource_list")))
