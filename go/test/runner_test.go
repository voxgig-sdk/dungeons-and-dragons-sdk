...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("DUNGEONSANDDRAGONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("DUNGEONSANDDRAGONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("DUNGEONSANDDRAGONS_TEST_EXPLAIN"); explain != "" {
