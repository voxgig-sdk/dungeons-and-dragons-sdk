...
boru-driven command-line client **and** interactive REPL for the DungeonsAndDragons
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dungeons-and-dragons-cli)
make build

# 2. See usage (words, entities, env vars)
./dungeons-and-dragons-cli --help

# 3. Provide credentials once, via the environment
export DUNGEONS_AND_DRAGONS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./dungeons-and-dragons-cli load 1 get_api_root            # {id:1} shorthand
./dungeons-and-dragons-cli load '{id:1}' get_api_root       # explicit match map

# 5. Override the API base URL for a single call
DUNGEONS_AND_DRAGONS_BASE=https://api.example.com ./dungeons-and-dragons-cli load 1 get_api_root

# 6. No arguments -> interactive REPL
./dungeons-and-dragons-cli
dungeons-and-dragons> load 1 get_api_root
dungeons-and-dragons> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dungeons-and-dragons-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dungeons-and-dragons-cli load 1 get_api_root
   ```
...
```sh
./dungeons-and-dragons-cli load 1 get_api_root          # scalar shorthand for {id:1}
./dungeons-and-dragons-cli load '{id:1}' get_api_root     # explicit match map
```
...
./dungeons-and-dragons-cli load 1 get_api_root
```
...
Run with no arguments to open a REPL (prompt `dungeons-and-dragons>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_api_root`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dungeons-and-dragons-cli`. |
...
(`./dungeons-and-dragons-cli <expr>`) and interactively (the REPL), and expressions compose the
