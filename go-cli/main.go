// Command DungeonsAndDragons-cli is a CLI + REPL for the DungeonsAndDragons SDK,
...
const prompt = "dungeons-and-dragons"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_api_root get_resource_by_index get_resource_list graph_ql"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DUNGEONS_AND_DRAGONS_APIKEY (api key), DUNGEONS_AND_DRAGONS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DUNGEONS_AND_DRAGONS_APIKEY carries the API key and
	// DUNGEONS_AND_DRAGONS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DUNGEONS_AND_DRAGONS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DUNGEONS_AND_DRAGONS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDungeonsAndDragonsSDK(opts)
