...
// DungeonsAndDragons SDK retry feature
...
class DungeonsAndDragonsRetryFeature extends DungeonsAndDragonsBaseFeature
{
    ...
    public function init(DungeonsAndDragonsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DungeonsAndDragonsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DungeonsAndDragonsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DungeonsAndDragonsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
