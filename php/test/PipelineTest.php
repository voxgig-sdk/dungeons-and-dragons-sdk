...
// DungeonsAndDragons SDK pipeline test
//
...
{
    private static function utility(): DungeonsAndDragonsUtility
    {
        return new DungeonsAndDragonsUtility();
    }
    ...
    private static function ctx(array $over = []): DungeonsAndDragonsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DungeonsAndDragonsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DungeonsAndDragonsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DungeonsAndDragonsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DungeonsAndDragonsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DungeonsAndDragonsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DungeonsAndDragonsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DungeonsAndDragonsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DungeonsAndDragonsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DungeonsAndDragonsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DungeonsAndDragonsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DungeonsAndDragonsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DungeonsAndDragonsResponse([]),
            'result' => new DungeonsAndDragonsResult([]),
        ]);
        [, $err] = DungeonsAndDragonsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DungeonsAndDragonsSpec([]),
            'response' => null,
            'result' => new DungeonsAndDragonsResult([]),
        ]);
        [, $err] = DungeonsAndDragonsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DungeonsAndDragonsSpec(['step' => 's']),
            'response' => new DungeonsAndDragonsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DungeonsAndDragonsResult([]),
        ]);
        [, $err] = DungeonsAndDragonsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DungeonsAndDragonsSpec(['step' => 's']),
            'response' => new DungeonsAndDragonsResponse(self::resp(200, ['v' => 1])),
            'result' => new DungeonsAndDragonsResult([]),
        ]);
        [, $err] = DungeonsAndDragonsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DungeonsAndDragonsSpec(['step' => 's']),
            'response' => new DungeonsAndDragonsResponse(self::resp(200, ['v' => 2])),
            'result' => new DungeonsAndDragonsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DungeonsAndDragonsMakeResponse::call($ctx);
    ...
    {
        $preset = new DungeonsAndDragonsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DungeonsAndDragonsSpec([]),
            'response' => new DungeonsAndDragonsResponse([]),
            'result' => new DungeonsAndDragonsResult([]),
        ...
        [$got, $err] = DungeonsAndDragonsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DungeonsAndDragonsResult([])]);
        [, $err] = DungeonsAndDragonsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DungeonsAndDragonsSpec([]), 'result' => null]);
        [, $err] = DungeonsAndDragonsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DungeonsAndDragonsSpec(['step' => 's']),
            'result' => new DungeonsAndDragonsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DungeonsAndDragonsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DungeonsAndDragonsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DungeonsAndDragonsSpec(['step' => 's']),
            'result' => new DungeonsAndDragonsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DungeonsAndDragonsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DungeonsAndDragonsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DungeonsAndDragonsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DungeonsAndDragonsSpec([]),
            'result' => new DungeonsAndDragonsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DungeonsAndDragonsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DungeonsAndDragonsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DungeonsAndDragonsError('boom', 'boom');
        $utility->fetcher = function (DungeonsAndDragonsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DungeonsAndDragonsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DungeonsAndDragonsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DungeonsAndDragonsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DungeonsAndDragonsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DungeonsAndDragonsResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        DungeonsAndDragonsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DungeonsAndDragonsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DungeonsAndDragonsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DungeonsAndDragonsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DungeonsAndDragonsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DungeonsAndDragonsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DungeonsAndDragonsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DungeonsAndDragonsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DungeonsAndDragonsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DungeonsAndDragonsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DungeonsAndDragonsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DungeonsAndDragonsResult(['ok' => false])]);
        $this->expectException(DungeonsAndDragonsError::class);
        DungeonsAndDragonsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DungeonsAndDragonsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DungeonsAndDragonsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DungeonsAndDragonsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DungeonsAndDragonsBaseFeature();
        $b = new DungeonsAndDragonsBaseFeature();
        DungeonsAndDragonsFeatureAdd::call($ctx, $a);
        DungeonsAndDragonsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DungeonsAndDragonsBaseFeature
    {
        $f = new DungeonsAndDragonsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DungeonsAndDragonsFeatureAdd::call($ctx, self::named_feature('a'));
        DungeonsAndDragonsFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        DungeonsAndDragonsFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        DungeonsAndDragonsFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        DungeonsAndDragonsFeatureAdd::call($ctx, $replace);
        ...
        DungeonsAndDragonsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DungeonsAndDragonsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DungeonsAndDragonsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DungeonsAndDragonsPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        DungeonsAndDragonsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DungeonsAndDragonsResponse(['headers' => null]),
            'result' => new DungeonsAndDragonsResult([]),
        ]);
        DungeonsAndDragonsResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        DungeonsAndDragonsResultBody::call($ctx);
