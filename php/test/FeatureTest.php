...
// DungeonsAndDragons SDK feature test
//
...
class FtCtrl extends DungeonsAndDragonsControl
{
...
class FtClient
{
    ...
    private DungeonsAndDragonsUtility $utility;

    public function __construct(DungeonsAndDragonsUtility $utility, array $options)
    {
    ...
    public function get_utility(): DungeonsAndDragonsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DungeonsAndDragonsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DungeonsAndDragonsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DungeonsAndDragonsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DungeonsAndDragonsConfig::make_feature($name);
    ...
    private static function buildUrl(DungeonsAndDragonsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DungeonsAndDragonsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DungeonsAndDragonsResponse($fetched);
        }
    ...
    private function fail(DungeonsAndDragonsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DungeonsAndDragonsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
