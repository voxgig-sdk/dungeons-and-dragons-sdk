...
// DungeonsAndDragons SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = DungeonsAndDragonsOmni::makeStructRunner(
    ...
    {
        return DungeonsAndDragonsOmni::objmodel(DungeonsAndDragonsOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return DungeonsAndDragonsOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return DungeonsAndDragonsOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(DungeonsAndDragonsOmni::structfix($now, Struct::undef()))
                        !== json_encode(DungeonsAndDragonsOmni::structfix($val, Struct::undef()))) {
