...
class GetResourceListDirectTest extends TestCase
{
    public function test_direct_list_get_resource_list(): void
    {
        $setup = get_resource_list_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-get_resource_list", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["get_resource_list01"] as $_liveKey) {
