...
local GetApiRootEntity = {}
GetApiRootEntity.__index = GetApiRootEntity


function GetApiRootEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetApiRootEntity)
  self._name = "get_api_root"
  self._client = client
...
function GetApiRootEntity:get_name()
  return self._name
end


function GetApiRootEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetApiRootEntity.new(self._client, opts)
end
...
function GetApiRootEntity:mark_deleted()
  self._deleted = true
end


function GetApiRootEntity:deleted()
  return true == self._deleted
end


function GetApiRootEntity:data_set(args)
  if args ~= nil then
...
function GetApiRootEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetApiRootEntity:match_set(args)
  if args ~= nil then
...
function GetApiRootEntity:match_get()
...
function GetApiRootEntity:stream(action, args, callopts)
...
---@param reqmatch GetApiRootLoadMatch
---@param ctrl? table
---@return GetApiRoot
---@return string? err
function GetApiRootEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GetApiRootEntity:_run_op(ctx, post_done)
  local utility = self._utility
