...
require("dungeons-and-dragons_types")
...
local DungeonsAndDragonsSDK = {}
DungeonsAndDragonsSDK.__index = DungeonsAndDragonsSDK
...
DungeonsAndDragonsSDK._make_feature = _make_feature


function DungeonsAndDragonsSDK.new(options)
  local self = setmetatable({}, DungeonsAndDragonsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DungeonsAndDragonsSDK:options_map()
...
function DungeonsAndDragonsSDK:get_utility()
  return Utility.copy(self._utility)
end


function DungeonsAndDragonsSDK:get_root_ctx()
  return self._rootctx
end


function DungeonsAndDragonsSDK:prepare(fetchargs)
...
function DungeonsAndDragonsSDK:direct(fetchargs)
...
function DungeonsAndDragonsSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function DungeonsAndDragonsSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "DungeonsAndDragonsSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function DungeonsAndDragonsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DungeonsAndDragonsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DungeonsAndDragonsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DungeonsAndDragonsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DungeonsAndDragonsSDK.new(sdkopts)
  sdk.mode = "test"
