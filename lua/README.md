...
The Lua SDK for the DungeonsAndDragons API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetApiRoot(nil):load({ id = "example_id" }, nil)
...
local result, err = client:DungeonsAndDragons(nil):load(
  { id = "test01" }, nil
)
